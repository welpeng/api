...
public class AltServiceImpl implements AltService {
    ...
    public List<String> formatConversion(String urlCookie) throws IOException {
        ...
        sidKey = cookie;        //匹配sidKey
        String regSID = "SID=[0-9,a-z，A-Z]+";
        Pattern pSID = Pattern.compile(regSID);
        Matcher mSID = pSID.matcher(sidKey);
        if (mSID.find()) {
            sidKey = (mSID.group());
        } else {
            return null;
        }
        sidKey = sidKey.substring(4,sidKey.length());
...
//        }
        ...
        altDao.insertCookie(cookie.get(0), cookie.get(1), "133" + (int) ((Math.random() * 90000000 + 9999999)), cookie.get(2));
        return "成功啦!\n谢谢你(´-｀ )";
